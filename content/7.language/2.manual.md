...
There are eight basic types in Cobalt: _null_, _boolean_, _number_, _string_, _function_, _userdata_, _thread_, and _table_ with a subset _array_. The type _null_ has one single value, **null**, whose main property is to be different from any other value; it often represents the absence of a useful value. The type _boolean_ has two values, **false** and **true**. Both **null** and **false** make a condition false; they are collectively called _false values_. Any other value makes a condition true. Despite its name, **false** is frequently used as an alternative to **null**, with the key difference that **false** behaves like a regular value in a table, while a **null** in a table represents an absent key.
...
The type _table_ implements associative arrays, that is, arrays that can have as indices not only numbers, but any Cobalt value except **null** and NaN. (_Not a Number_ is a special floating-point value used by the IEEE 754 standard to represent undefined numerical results, such as `0/0`.) Tables can be _heterogeneous_; that is, they can contain values of all types (except **null**). Any key associated to the value **null** is not considered part of the table. Conversely, any key that is not part of a table has an associated value **null**.
...
The following _keywords_ are reserved and cannot be used as names:
     ...
     local     null       not       or        repeat    return
...
The condition expression of a control structure can return any value. Both **false** and **null** test false. All values different from **null** and **false** test true. In particular, the number 0 and the empty string also test true.
...
Then, at each iteration, Cobalt calls the iterator function with two arguments: the state and the control variable. The results from this call are then assigned to the loop variables, following the rules of multiple assignments (see [§3.3.3](#3.3.3)). If the control variable becomes **null**, the loop terminates. Otherwise, the body is executed and the loop goes to the next iteration.
...
Here, to _close_ a value means to call its `__close` metamethod. When calling the metamethod, the value itself is passed as the first argument and the error object that caused the exit (if any) is passed as a second argument; if there was no error, the second argument is **null**.

The value assigned to a to-be-closed variable must have a `__close` metamethod or be a false value. (**null** and **false** are ignored as to-be-closed values.)
...
The logical operators in Cobalt are **and**, **or**, and **not**. Like the control structures (see [§3.3.4](#3.3.4)), all logical operators consider both **false** and **null** as false and anything else as true.

The negation operator **not** always returns **false** or **true**. The conjunction operator **and** returns its first argument if this value is **false** or **null**; otherwise, **and** returns its second argument. The disjunction operator **or** returns its first argument if this value is different from **null** and **false**; otherwise, **or** returns its second argument. Both **and** and **or** use short-circuit evaCobalttion; that is, the second operand is evaCobaltted only if necessary. Here are some examples:
...
The length operator applied on a table returns a border in that table. A _border_ in a table `t` is any non-negative integer that satisfies the following condition:
     ...
     (border == 0 or t\[border\] ~= null) and
     (t\[border + 1\] == null or border == math.maxinteger)
...
A table with exactly one border is called a _sequence_. For instance, the table `{10, 20, 30, 40, 50}` is a sequence, as it has only one border (5). The table `{10, 20, 30, null, 50}` has two borders (3 and 5), and therefore it is not a sequence. (The **null** at index 4 is called a _hole_.) The table `{null, 20, 30, null, null, 60, null}` has three borders (0, 3, and 6), so it is not a sequence, too. The table `{}` is a sequence with border 0.
...
Cobalt uses a _virtual stack_ to pass values to and from C. Each element in this stack represents a Cobalt value (**null**, number, string, etc.). Functions in the API can access this stack through the Cobalt state parameter that they receive.
...
Close the to-be-closed slot at the given index and set its value to **null**. The index must be the last index previously marked to be closed (see [`lua_toclose`](#lua_toclose)) that is still active (that is, not closed yet).
...
If the userdata does not have that value, pushes **null** and returns [`lua_TNONE`](#pdf-lua_TNONE).
...
If the resulting function has upvalues, its first upvalue is set to the value of the global environment stored at index `lua_RIDX_GLOBALS` in the registry (see [§4.3](#4.3)). When loading main chunks, this upvalue will be the `_ENV` variable (see [§2.2](#2.2)). Other upvalues are initialized with **null**.
...
This function may raise an error if the given key is neither **null** nor present in the table. See function [`next`](#pdf-next) for the caveats of modifying the table during its traversal.
...
void lua\_pushnull (lua\_State \*L);
...
Pops a table or **null** from the stack and sets that value as the new metatable for the value at the given index. (**null** means no metatable.)
...
Accepts any index, or 0, and sets the stack top to this index. If the new top is greater than the old one, then the new elements are filled with **null**. If `index` is 0, then all stack elements are removed.
...
Converts the Cobalt value at the given index to a C boolean value (0 or 1). Like all tests in Cobalt, [`lua_toboolean`](#lua_toboolean) returns true for any Cobalt value different from **false** and **null**; otherwise it returns false. (If you want to accept only actual boolean values, use [`lua_isboolean`](#lua_isboolean) to test the value's type.)
...
Returns the type of the value in the given valid index, or `lua_TNONE` for a non-valid but acceptable index. The types returned by [`lua_type`](#lua_type) are coded by the following constants defined in `cobalt.h`: `lua_Tnull`, `lua_TNUMBER`, `lua_TBOOLEAN`, `lua_TSTRING`, `lua_TTABLE`, `lua_TFUNCTION`, `lua_TUSERDATA`, `lua_TTHREAD`, and `lua_TLIGHTUSERDATA`.
...
Pushes onto the stack the metatable associated with the name `tname` in the registry (see [`luaL_newmetatable`](#luaL_newmetatable)), or **null** if there is no metatable associated with that name. Returns the type of the pushed value.
...
In words, if the argument `arg` is null or absent, the macro results in the default `dflt`. Otherwise, it results in the result of calling `func` with the state `L` and the argument index `arg` as arguments. Note that it evaCobalttes the expression `dflt` only if needed.
...
If the function argument `arg` is an integer (or it is convertible to an integer), returns this integer. If this argument is absent or is **null**, returns `d`. Otherwise, raises an error.
...
If the function argument `arg` is a number, returns this number as a `lua_Number`. If this argument is absent or is **null**, returns `d`. Otherwise, raises an error.
...
If the object on the top of the stack is **null**, [`luaL_ref`](#luaL_ref) returns the constant `lua_REFnull`. The constant `lua_NOREF` is guaranteed to be different from any reference returned by [`luaL_ref`](#luaL_ref).
...
The notation **fail** means a false value representing some kind of failure. (Currently, **fail** is equal to **null**, but that may change in future versions. The recommendation is to always test the success of these functions with `(not status)`, instead of `(status == null)`.)
...
Raises an error if the value of its argument `v` is false (i.e., **null** or **false**); otherwise, returns all its arguments. In case of error, `message` is the error object; when absent, it defaults to "`assertion failed!`"
...
Regardless, if the resulting function has any upvalues, its first upvalue is set to the value of `env`, if that parameter is given, or to the value of the global environment. Other upvalues are initialized with **null**. All upvalues are fresh, that is, they are not shared with any other function.
...
You should not assign any value to a non-existent field in a table during its traversal. You may however modify existing fields. In particular, you may set existing fields to null.
...
Sets the real value of `table[index]` to `value`, without using the `__newindex` metavalue. `table` must be a table, `index` any value different from **null** and NaN, and `value` any Cobalt value.
...
Sets the metatable for the given table. If `metatable` is **null**, removes the metatable of the given table. If the original metatable has a `__metatable` field, raises an error.
...
Each entry in this table is a _searcher function_. When looking for a module, [`require`](#pdf-require) calls each of these searchers in ascending order, with the module name (the argument given to [`require`](#pdf-require)) as its sole argument. If the searcher finds the module, it returns another function, the module _loader_, plus an extra value, a _loader data_, that will be passed to that loader and returned as a second result by [`require`](#pdf-require). If it cannot find the module, it returns a string explaining why (or **null** if it has nothing to say).
...
The specifier `q` formats booleans, null, numbers, and strings in a way that the result is a valid constant in Cobalt source code. Booleans and null are written in the obvious way (`true`, `false`, `null`). Floats are written in hexadecimal, to preserve full precision. A string is written between double quotes, using escape sequences when necessary to ensure that it can safely be read back by the Cobalt interpreter. For instance, the call
...
The specifier `p` formats the pointer returned by [`lua_topointer`](#lua_topointer). That gives a unique string identifier for tables, userdata, threads, strings, and functions. For other values (numbers, null, booleans), this specifier results in a string representing the pointer `NULL`.
...
Returns a new table with all arguments stored into keys 1, 2, etc. and with a field "`n`" with the total number of arguments. Note that the resulting table may not be a sequence, if some arguments are **null**.
...
Opens the given file name in read mode and returns an iterator function that works like `file->lines(···)` over the opened file. When the iterator function fails to read any value, it automatically closes the file. Besides the iterator function, `io.lines` returns three other values: two **null** values as placeholders, plus the created file handle. Therefore, when used in a generic **for** loop, the file is closed also if the loop is interrupted by an error or a **break**.
